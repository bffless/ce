...
| Adapter | Presigned uploads | URL shape | Mechanism |
| --- | --- | --- | --- |
| Local | ✅ (browser callers only — see below) | Relative by default, e.g. `/api/storage/presigned/local?key=...&exp=...&max=...&sig=...`; absolute only when `PUBLIC_ORIGIN` is set | HMAC-SHA256-signed `PUT`, served on the app's own host |
| MinIO / S3 / GCS / Azure | ✅ | Always absolute (provider URL) | Provider-native presigned URL |

`LocalStorageAdapter.getPresignedUploadUrl(key, expiresIn?)` (`local.adapter.ts`) mints a URL for
`PUT /api/storage/presigned/local`, implemented by `local-presigned-upload.controller.ts`. Two
things about it are easy to get wrong, because an earlier revision of this design got them wrong
too — see `docs/superpowers/specs/2026-07-30-local-fs-presigned-uploads-design.md`, section
"Correction: upload URL routing", for the full story.

**The URL is relative by default, not absolute.** With no `PUBLIC_ORIGIN` configured, the adapter
returns a path like `/api/storage/presigned/local?...`, and the browser resolves it against
whatever host served the page. `PUBLIC_ORIGIN` is an **optional override** for callers that need
an absolute URL — it is never a precondition for the feature to work, and there is no fallback to
`PRIMARY_DOMAIN`. (An earlier revision of this design *did* fall back to
`https://${PRIMARY_DOMAIN}`, and that fallback fired unconditionally in production because
`PRIMARY_DOMAIN` is always set — minting a URL at the apex domain, which has no nginx location
that proxies `/api` to the backend. That fallback has been removed; `PUBLIC_ORIGIN` is read from
`PUBLIC_ORIGIN` alone.)

**"No CORS needed" is real, but it's earned by routing, not free.** It holds only because the
presign route is served on the app's own host via dedicated, unrewritten nginx locations —
`location = /api/storage/presigned/local` in `apps/backend/templates/nginx/subdomain.conf.hbs`,
`custom-domain.conf.hbs`, the admin block generated by `generateCEPrimaryDomainConfig` in
`nginx-config.service.ts`, and the admin block of `docker/nginx/sites-available/main.conf.template`.
Each of those locations sets `proxy_request_buffering off` (so the request streams through nginx
rather than being spooled) and its own `client_max_body_size 200M` (the `docker/nginx/nginx.conf`
http-level default is `10M` and does not apply here). If that routing is ever moved or the
locations dropped, the minted relative URL stops resolving to the backend and the same-origin
property silently breaks — there's no code-level dependency enforcing it.
(`docker/nginx.conf` at the repo root is unused legacy; the live configs are under
`docker/nginx/`.)

**`supportsPresignedUrls()` requires real signing material, not an origin.** It returns `true`
only when there is a real signing secret: an explicitly-passed `presignKey`, or
`LOCAL_PRESIGN_SECRET`/`ENCRYPTION_KEY` set in the environment (`hasRealPresignSecret` in
`presign.util.ts`). With neither set, key derivation still succeeds (it falls back to a hardcoded
dev-only secret so construction never throws), but `supportsPresignedUrls()` deliberately reports
`false` in that case — the fallback secret is a public constant, so treating it as real signing
material would mean anyone could forge upload signatures. `ENCRYPTION_KEY` is mandatory in CE
setup, so this should never trigger on a real install; it exists as a fail-closed backstop.

**Local presigned uploads are a browser feature — CI/server callers don't get them.**
`POST /api/deployments/prepare-batch-upload` and `prepare-batch-download`
(`deployments.controller.ts`) deliberately treat the local adapter as **not** supporting presigned
uploads, regardless of what `supportsPresignedUrls()` reports, and fall back to the ZIP-upload /
`/api/files/...` path instead. A relative URL needs a page origin to resolve against; a Node/CI
client (the `upload-artifact` GitHub Action, the `bffless` CLI) and even the admin SPA have no
