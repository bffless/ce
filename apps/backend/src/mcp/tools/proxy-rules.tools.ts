...
- file_delete: { prefix?: "expression" (relative to the project's uploads root, e.g. "projects/{{request.body.projectId}}/"), key?: "expression" (single object, relative to uploads root), dryRun?: bool }. DESTRUCTIVE. Provide exactly ONE of prefix or key. prefix mode lists+deletes EVERY object under <uploadsRoot>/<prefix> (object stores have no atomic folder delete); key mode deletes one object. Both are relative to {owner}/{repo}/uploads/ — the same root file_upload/file_serve use via subDir — and are interpolated before use. Idempotent: a prefix/key matching nothing returns { deleted: 0 } (not an error). SAFETY: a blank/whitespace/"/"-only prefix and any ".." traversal are rejected before any storage call (a blank prefix never means "delete everything"). Output: prefix mode { deleted: number, prefix, dryRun }; key mode { deleted: 0|1, key, dryRun }. Since this is destructive, gate the consuming rule with the auth_required validator once auth is in place.
