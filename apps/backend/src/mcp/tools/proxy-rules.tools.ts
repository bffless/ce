...
- ffmpeg_handler: {operation: 'probe'|'extract_audio'|'slice'|'concat', input?, inputs?: string[], spans?: [{start,end}], output?, audioOutput?, audioFades?, executor?: 'local'|'remote'|'{{expression}}' (default: the instance's default executor)} — server-side video ops on storage objects via guarded native ffmpeg. probe with no input returns {server,ops,version,executors,defaultExecutor,remote?} (capability check, never fails; version is the LOCAL ffmpeg's, the worker's is remote.version). extract_audio → 16kHz mono WAV. slice cuts kept spans into one clip (+optional WAV). concat stitches uniform clips (stream-copy, auto re-encode fallback). Paths are uploads-relative or /api/uploads/... URLs. Heavy ops belong in postSteps with a job row (fire-and-poll); outputs are {storage_path, content_type, size} plus {executor, timings, bytesIn, bytesOut}. Errors: FFMPEG_UNAVAILABLE (server video ops are switched OFF for this instance) | FFMPEG_EXECUTOR_UNAVAILABLE (they are ON, but the executor the step asked for — or the instance default — is not configured or not ready: no ffmpeg binaries for 'local', no FFMPEG_REMOTE_URL / unreachable worker for 'remote') | FFMPEG_BUSY (at capacity — retry later) | FFMPEG_TIMEOUT|FFMPEG_JOB_TIMEOUT|FFMPEG_FAILED|... Fall back to client-side processing when probe reports server:false. Server video ops are opt-in per instance (Admin Settings → Features); probe reports server:false until an admin enables them.
