...
- ffmpeg_handler: {operation: 'probe'|'extract_audio'|'slice'|'concat'|'frames'|'contact_sheet', input?, inputs?: string[], spans?: [{start,end}], output?, audioOutput?, audioFades?, outputPrefix?, times?: (number|expression)[]|expression, height?: number, quality?: number, duration?: number|expression, interval?: number, columns?: number, cellsPerSheet?: number, maxSheets?: number, label?: boolean, executor?: 'local'|'remote'|'{{expression}}' (default: the instance's default executor)} — server-side video ops on storage objects via guarded native ffmpeg. probe with no input returns {server,ops,version,executors,defaultExecutor,remote?: {ready, version?, maxInflight, reason?}} (capability check, never fails; version is the LOCAL ffmpeg's, the worker's is remote.version, remote.maxInflight is the connection's concurrency cap). extract_audio → 16kHz mono WAV. slice cuts kept spans into one clip (+optional WAV). concat stitches uniform clips (stream-copy, auto re-encode fallback). frames writes one CLEAN (unlabelled) still per entry in times to <outputPrefix>/frame-NN.jpg (1-based, zero-padded to 2 digits and wider past 99) and returns {frames:[{time,storage_path,content_type,size}],count}, where storage_path is the FULL resolved key {owner}/{repo}/uploads/<prefix>/frame-01.jpg (same convention as slice/extract_audio), not the outputPrefix as written — a time past the end of the source FAILS the step, and it is the ONE case that leaves a partial batch behind (ffmpeg exits 0 writing nothing, so the failure surfaces in the upload loop after earlier stills landed). contact_sheet samples the whole clip and tiles the samples into <outputPrefix>/sheet-NN.jpg grids an LLM can read, returning {sheets:[{storage_path,content_type,size,times,index,total,cols,rows}],interval,count,labelled} (storage_path is the full resolved key like frames; sheets[].index is 0-based while the filename is 1-based and zero-padded, widening past 99); labelled is the OUTCOME not the diagnosis — it is false when label:false was set as well as when the ffmpeg lacked drawtext; duration is probed with ffprobe when omitted, which downloads the source a SECOND time, so pass a known duration for large sources; each cell burns in an m:ss clock via ffmpeg's drawtext, and an ffmpeg without that filter degrades to un-labelled cells and labelled:false instead of failing. frames/contact_sheet write under outputPrefix (a DIRECTORY) instead of a single output; treat a run's prefix as disposable. Otherwise nothing partial is left: both executors upload only after EVERY command succeeded, so a non-zero ffmpeg exit uploads nothing. Defaults: height 720, quality 3 (frames only; jpeg -q:v, 2=best 31=worst — NOT clamped, quality:500 reaches ffmpeg verbatim), interval 5s, columns 3, cellsPerSheet 12, maxSheets 10, label true. THREE syntaxes, do not mix them: every path/name field — input, output, audioOutput, outputPrefix and executor — is a TEMPLATE ({{steps.x.y}} substituted, anything else used verbatim); times/duration are BARE expressions (write steps.probe.duration — a value starting {{ is returned as a LITERAL string, so duration:'{{steps.probe.duration}}' fails INVALID_TIMES and times:'{{...}}' fails 'expected a non-empty array'); height/quality/interval/columns/cellsPerSheet/maxSheets are LITERAL numbers resolved not at all, so height:'{{steps.x.h}}' is a config error and a bare steps.x.h is not resolved either. At most 200 stills per step, and BOTH checks are run-time only — CE has no save-time handler validation, so an over-budget step saves cleanly and fails on its first request (validateConfig runs immediately before execute, never on write). contact_sheet's maxSheets x cellsPerSheet is static so it surfaces as a ConfigurationError; frames' times.length is expression-resolved, so it is only knowable once the step runs. A contact sheet is up to 131 ffmpeg/ffprobe commands AT THE DEFAULT CAPS (1 probe + 120 cells + 10 tiles) and more if you raise them (cellsPerSheet:20/maxSheets:10 is a legal 200-still budget = 211 commands), and the local runner takes its concurrency slot PER COMMAND, so it has many chances to hit FFMPEG_BUSY and a mid-job failure discards the cells already computed — run it in postSteps behind a job row. A missing drawtext costs a second full un-labelled pass only on the RETRY path (remote Worker, or a local whose -filters probe never ran); a local ffmpeg already probed as lacking it suppresses labels up front at zero extra cost. Paths are uploads-relative or /api/uploads/... URLs. Heavy ops belong in postSteps with a job row (fire-and-poll); outputs are {storage_path, content_type, size} (frames/contact_sheet: the frames/sheets arrays above) plus {executor, timings, bytesIn, bytesOut}. Errors: FFMPEG_UNAVAILABLE (server video ops are switched OFF for this instance) | FFMPEG_EXECUTOR_UNAVAILABLE (they are ON, but the executor the step asked for — or the instance default — is not configured or not ready: no ffmpeg binaries for 'local', no remote connection selected / unreachable worker for 'remote') | FFMPEG_BUSY (at capacity — retry later) | FFMPEG_TIMEOUT|FFMPEG_JOB_TIMEOUT|FFMPEG_FAILED|... Fall back to client-side processing when probe reports server:false. Server video ops are opt-in per instance (Admin Settings → Features); probe reports server:false until an admin enables them.
