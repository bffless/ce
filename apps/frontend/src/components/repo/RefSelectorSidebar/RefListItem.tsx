    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between gap-2">
          <span className={cn('font-medium truncate', type === 'commit' && 'font-mono text-sm')}>
            {name}
          </span>
        ...
        {/* Secondary info */}
        {type !== 'commit' && (
          <span className="text-xs text-muted-foreground font-mono">{sha.substring(0, 7)}</span>
        )}

        {/* Commit-specific info */}
        {type === 'commit' && (
          <>
            {branch && <div className="text-xs text-muted-foreground truncate">{branch}</div>}
            {description && (
