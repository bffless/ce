# Umbrel-specific backend image
# Same as backend.Dockerfile but with umbrel-specific entrypoint that:
# - Derives security keys from Umbrel's APP_SEED
# - Reads domain configuration from /app/config/domain.txt
...
# Copy UMBREL-SPECIFIC entrypoint script (different from standard backend)
COPY umbrel/umbrel-entrypoint.sh /app/entrypoint.sh
...
# Use umbrel entrypoint script to run migrations then start app
