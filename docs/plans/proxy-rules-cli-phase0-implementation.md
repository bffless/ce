...
2. Discover rules under `rules/`: files matching `(get|post|put|patch|delete|head|options|any).rule.yaml` (single-file shape) and directories `(get|post|…|any)/` containing `rule.yaml` (directory shape). Method = filename stem; `any` → no `method` key (manifest `methods:` allowed only in `any` rules — error otherwise). *(Amended after Task 6 review:)* an explicit manifest `method:` is honored as a single-method escape hatch in `any` rules (but not together with `methods:`); in a method-stem file it must match the stem (mismatch = error), and directory entries are sorted for deterministic discovery. File references (`code:`, `$file:`) are confined to the rule-set directory — absolute paths or `..` escapes are errors (path-traversal guard).
...
7. Schema refs: every value `$schema:<name>` under a `SCHEMA_REF_KEYS` key → resolved: read `schemas/<name>.schema.yaml`; uuid = its `id:` if present else uuidv5(name, namespace `6e1a24d0-0000-4000-8000-bff1e55c0de0`) *(amended: original namespace string contained a non-hex `l`)* computed via `node:crypto` (implement RFC-4122 v5 inline — ~15 lines, no dependency); collect each referenced schema once into `schemas[]` as `{ id, name, fields }`. A `$schema:` ref with no manifest file → error. A raw UUID value is passed through untouched with a warning (`unresolved schema id <uuid> in <file>`).
