...
- `canonicalizeExport`: deep-clone; drop every `undefined`/`null` value; order envelope keys per `ENVELOPE_KEY_ORDER`; order rule keys per `RULE_KEY_ORDER` (unknown keys are an error — throw with the key name); sort `rules` by `(order ?? 0, pathPattern, method ?? '')`; sort `schemas` by `name`; delete `schemas` when absent/empty. Nested objects (`pipelineConfig` etc.) keep their own insertion order except step keys normalized to `id,name,handlerType,config,isEnabled` (both orders occur in real fixtures).
- `stringifyExport`: `JSON.stringify(canonicalizeExport(e), null, 2) + '\n'`.
- `exportsEquivalent(a,b)`: compare `canonicalizeExport(a)` vs `canonicalizeExport(b)` with `exportedAt` deleted from both; walk recursively collecting dotted diff paths, using `rules[<pathPattern> <method>]` (not array index) as the rules path segment; `{ equal: diffs.length === 0, diffs }`.
...
- [ ] **Step 1: Write failing tests** — cases: (a) minimal pipeline manifest gains all defaults + inferred proxyType + default targetUrl; (b) explicit `proxyType:'external_proxy'` beats inference; (c) elide→apply is identity for every rule of every real fixture (import the fixtures as in Task 2 and assert `applyRuleDefaults(elideRuleDefaults(r))` deep-equals `canonicalizeExport`'d rule); (d) `targetUrl:"pipeline"` is preserved by elision; (e) `timeout:30000` dropped, `timeout:15000` kept.
