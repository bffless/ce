...
**As built (#796, streamed inputs):** an input may carry `stream: true`. The Worker then does not download it; `{in:NAME}` resolves to the signed URL itself and ffmpeg reads it with range requests, so a large source never lands in scratch (in-memory on Cloud Run). **CE owns the input options a streamed input needs**: `buildEnvelope` inserts `-reconnect 1 -reconnect_on_network_error 1 -reconnect_delay_max 5` immediately before the `-i` that precedes the streamed input's whole-token placeholder. `ffmpeg-args.ts` and the local executor are unchanged, and the Worker still only substitutes placeholders and runs argv verbatim. It never inspects or rewrites flags. CE's handler hints `stream` for slice, extract-audio and frames (including contact sheets); concat keeps downloading. CE sends `stream` (and the reconnect flags) only to a Worker whose `/health` reports `protocol >= 2`. That number is owned by the Worker rather than read off `version`, because image versions are CE tags (`v0.4.59`, `preview-…`, `dev`) and only some of them compare as semver. Older Workers report no `protocol` and keep receiving download-only envelopes. The envelope stays `v: 1` because the field is optional.
