...
generate_ssl_certificates() {
    ...
    if [ "$PROXY_MODE" = "cloudflare" ]; then
        ...
        if [ "$INTERACTIVE" = true ]; then
            ...
            echo "  2. Create DNS A records for: @ and * (wildcard covers all subdomains)"
...
${line}"
    ...
    echo "  ────    ────    ─────"
    printf "  A       @       <your-server-ip>  ${DIM}(root domain)${NC}\n"
    printf "  A       *       <your-server-ip>  ${DIM}(all subdomains: www, admin, minio, etc.)${NC}\n"
    echo ""
...
print_next_steps() {
    ...
    if [ "$PROXY_MODE" = "cloudflare" ]; then
        ...
        echo ""
        printf "     ${YELLOW}@${NC}   → A record → ${YELLOW}${DISPLAY_IP}${NC}  (Proxied) - root domain\n"
        printf "     ${YELLOW}*${NC}   → A record → ${YELLOW}${DISPLAY_IP}${NC}  (Proxied) - all subdomains\n"
        echo ""
        step=$((step + 1))
    ...
    # Standard Let's Encrypt next steps (only if not localhost and SSL wasn't generated)
        ...
        echo ""
        printf "     ${YELLOW}@${NC}   → A record → ${YELLOW}${DISPLAY_IP}${NC}  (root domain)\n"
        printf "     ${YELLOW}*${NC}   → A record → ${YELLOW}${DISPLAY_IP}${NC}  (all subdomains: www, admin, minio, etc.)\n"
        echo ""
...
run_bootstrap_mode() {
    ...
    echo ""
    printf "  ${CYAN}3.${NC} Open the setup wizard - this link carries your claim token, so it\n"
    printf "     skips straight to account setup (no claim screen to fill in):\n"
    echo ""
    printf "     ${YELLOW}https://admin.<your-domain>/setup?token=${CLAIM_TOKEN}${NC}\n"
    printf "     ${DIM}(or https://<server-ip>/setup?token=${CLAIM_TOKEN} - expect a browser certificate warning)${NC}\n"
    echo ""
    printf "  ${CYAN}4.${NC} Typed the URL in by hand instead? Paste the claim token above when the\n"
    printf "     wizard's claim-token screen asks for it.\n"
    echo ""
    exit 0
}
